...
printf '\e[2m******************* \e[0m\e[36mПрограмма управления сервисом Reset Lan\e[0m\e[2m ********************\e[0m\n'
...
EXIT_PROGRAM(){
################################## очистка на выходе #############################################################
cat  ~/.bash_history | sed -n '/ResetLanManager/!p' >> ~/new_hist.txt; rm ~/.bash_history; mv ~/new_hist.txt ~/.bash_history
#####################################################################################################################
...
if [[ $(launchctl list | grep "reset-lan.job" | cut -f3 | grep -x "reset-lan.job") ]]; then  rs_lan="работает"
        else
if [[ ! -f ~/Library/LaunchAgents/ResetLan.plist ]]; then rs_lan="не установлен"
            else
                 rs_lan="остановлен"
        fi
fi
...
printf '                    \e[1;32m     Сервис \e[0mReset Lan \e[1;32m'"$rs_lan"'\e[0m                   \n'
else
printf '                    \e[1;33m     Сервис \e[0mReset Lan \e[1;33m'"$rs_lan"'\e[0m                   \n'
fi
printf ' %.0s' {1..80}
printf '.%.0s' {1..80}
printf '\n'
printf ' %.0s' {1..80}
printf '          \e[1;33m1.\e[0m Установить сервис Reset Lan                 \n'
printf '          \e[1;33m2.\e[0m Остановить                   \n'
printf '          \e[1;33m3.\e[0m Запустить                 \n'
printf '          \e[1;33m4.\e[0m Удалить сервис Reset Lan                \n'
...
if [[ $inputs = 1 ]]; then
    ...
    if [[ $PASSWORD = "" ]]; then printf '\n\n  \e[1;31mТри неверных ввода пароля ! \e[0m\n'; sleep 2
            ...
            if [[ -f tools/ResetLan.plist ]] && [[ -f tools/ResLan.sh ]]; then
                if [[ ! -f ~/Library/LaunchAgents/ResetLan.plist ]]; then cp -a tools/ResetLan.plist ~/Library/LaunchAgents; fi
                plutil -remove ProgramArguments.0 ~/Library/LaunchAgents/ResetLan.plist
                plutil -insert ProgramArguments.0 -string "/Users/$(whoami)/.ResLan.sh" ~/Library/LaunchAgents/ResetLan.plist
                if [[ ! -f ~/.ResLan.sh ]]; then cp -a tools/ResLan.sh ~/.ResLan.sh; chmod u+x ~/.ResLan.sh; fi
                if [[ ! $rs_lan = "работает" ]]; then launchctl load -w ~/Library/LaunchAgents/ResetLan.plist; fi
        else
...
if [[ $inputs = 2 ]]; then
    CHECK_RESLAN
    if [[ $rs_lan = "работает" ]]; then
        if [[ -f ~/Library/LaunchAgents/ResetLan.plist ]]; then
            launchctl unload -w ~/Library/LaunchAgents/ResetLan.plist; fi
        fi
    CLEAR_PLACE
fi
...
if [[ $inputs = 3 ]]; then
    ...
    if [[ $rs_lan = "остановлен" ]]; then
        if [[ -f ~/Library/LaunchAgents/ResetLan.plist ]]; then
            launchctl load -w ~/Library/LaunchAgents/ResetLan.plist; fi
        fi
    CLEAR_PLACE
fi
...
if [[ $inputs = 4 ]]; then
    CHECK_RESLAN
    if [[ -f ~/.auth/auth.plist ]]; then rm -R ~/.auth ; fi
    if [[ $(launchctl list | grep "reset-lan.job" | cut -f3 | grep -x "reset-lan.job") ]]; then launchctl unload -w ~/Library/LaunchAgents/ResetLan.plist; fi
    if [[ -f ~/Library/LaunchAgents/ResetLan.plist ]]; then rm ~/Library/LaunchAgents/ResetLan.plist; fi
    if [[ -f ~/.ResLan.sh ]]; then rm ~/.ResLan.sh; fi
    read -n 1 -t 1
    CLEAR_PLACE
fi
